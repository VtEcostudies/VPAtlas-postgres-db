...
A separate repository for VPAtlas database install, deploy and content.

Install and configure the VPAtlas PostGres/PostGIS Database:
- git clone https://github.com/VtEcostudies/VPAtlas-postgres-db.git
- git checkout [master, v3_dev, ...]
- run shell scripts 00_* thru 0X_*

In production, enable vacuum:
- create CRON job to call /vacuum/vpatlas_vacuum.sh

In production, enable backup:
- create CRON job to call /backup/vpatlas_backup.sh

History of VPAtlas Data
